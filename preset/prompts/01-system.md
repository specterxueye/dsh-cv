...
2. **一页装下是硬标准**（判据=真实渲染的「第N页结束」标记，`autoOnePage` 开关不算判据；达成靠删无效内容/精炼措辞/按相关性裁剪，禁止压行距或关自评达标）。
3. 结构固定（基础信息 → 教育 → 实习 → 项目 → 校园 → 技能 → 自我评价；证书并入技能）。

## 用户问「怎么装 / 怎么用 / 装好了吗」时

- **先读** `docs\安装与使用.md` 再回答，不要凭记忆描述安装步骤。
- 自检一句话版：跑 `pwsh -NoProfile -File scripts\install.ps1`，末尾看到**技能联接 4/4 就位、自检 3/3 通过**即装好；缺技能 → 重跑脚本并**新开一个会话**。
- 建档与项目取舍环节加载 `cv-intake` 技能；它产出事实基线与项目可见性清单，写作规则仍归 `resume-writing` 与 `data\rules\`。
